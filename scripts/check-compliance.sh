...
#
# md2docx has no cross-repo rebar contract dependencies today (see
# CONSUMES.md), so this check is present for forward-compatibility only —
# it activates automatically if that ever changes.
...
if [ -f "$CONSUMES_FILE" ]; then
  ...
  # NOTE: `|| true` here, not `|| echo 0` — grep -c prints "0" and still
  # exits 1 when there are zero matches, and under `set -o pipefail` that
  # makes the pipeline "fail" even though tr already emitted "0" into the
  # command substitution. `|| echo 0` would then append a SECOND "0",
  # producing the string "00". `|| true` just accepts the already-correct
  # "0" that tr produced.
  consumes_entries=$(grep -cE '^## [A-Za-z0-9_-]+/[A-Za-z0-9_.-]+$' "$CONSUMES_FILE" 2>/dev/null | tr -d '[:space:]' || true)
  if [ "${consumes_entries:-0}" = "0" ]; then
    ...
    if [ "$drift_wired" -eq 1 ]; then
      ...
      echo "  Add to scripts/pre-commit.sh (or your CI of choice):"
